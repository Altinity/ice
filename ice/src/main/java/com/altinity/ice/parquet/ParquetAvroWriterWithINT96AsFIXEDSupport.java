...
public class ParquetAvroWriterWithINT96AsFIXEDSupport {
  ...
  public <T> ParquetValueWriter<T> buildWriter(MessageType type) {
      ...
      if (primitive.getOriginalType() != null) {
        switch (primitive.getOriginalType()) {
          ...
          case TIMESTAMP_MICROS:
            if (int96fields != null) {
              String p = String.join(".", currentPath());
              if (int96fields.contains(p)) {
                return new INT96FixedWriter(desc);
              }
            }
  ...
  private static class INT96FixedWriter extends PrimitiveWriter<Fixed> {
    private INT96FixedWriter(ColumnDescriptor desc) {
      super(desc);
    }

    @Override
    public void write(int repetitionLevel, Fixed buffer) {
      // INT96 = [nanoseconds_since_midnight (8 bytes)] + [julian_day (4 bytes)] (little-endian)
      ByteBuffer buf = ByteBuffer.wrap(buffer.bytes()).order(ByteOrder.LITTLE_ENDIAN);
      column.writeLong(repetitionLevel, ParquetUtil.extractTimestampInt96(buf));
    }
  }
